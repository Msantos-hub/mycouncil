                        ...
                        <template v-slot:default>
                            ...
                            <tbody>
                                <tr v-for="(sugerencia,index) of sugerencia" :key="index">
                                    ...
                                    <td>
                                        <v-btn color="success" dark rounded @click="aceptDialog= true,selectSugerencia(sugerencia)">
                                            <v-icon>mdi-check</v-icon>    
                                        </v-btn>
                                        <v-btn color="error" rounded class="ml-5"
                                            @click="deleteDialog= true, selectSugerencia(sugerencia)">
    ...
    <!-- aceptar Sugerencia-->   
        <v-dialog persistent max-width="350" v-model="aceptDialog">
            ...
            <v-card-text>
                ...
                <h3>Esta Aceptando la Sugerencia "Nº {{currentSugerencia.idSugerencia}}"</h3>
            </v-card-text>
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn
        ...
        <v-card-text>
            ...
            <h3>Esta borrando la Sugerencia "Nº {{currentSugerencia.idSugerencia}}"</h3>
        </v-card-text>
        <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn
...
<script>
    ...
    methods: {
        ...
        async getSugerencias(){
            const _this=this
            await axios.get("http://localhost/proyecto/mycouncil/src/bbdd/sugerencias.php?accion=leer").then(function(response){
        ...
        async getSugerencias2(){
            const _this=this
            await axios.get("http://localhost/proyecto/mycouncil/src/bbdd/sugerencias.php?accion=leer2").then(function(response){
        ...
        updateSugerencia(){
            ...
            axios.post("http://localhost/proyecto/mycouncil/src/bbdd/sugerencias.php?accion=aprobar",formData).then(function(response){
        ...
        deleteSugerencia(){
            const _this = this
            var formData = _this.toFormData(_this.currentSugerencia);
            axios.post("http://localhost/proyecto/mycouncil/src/bbdd/sugerencias.php?accion=borrar",formData).then(function(response){
                _this.currentSugerencia={};
                if(response.data.error){                   
                    _this.errorMsg=response.data.message;
                }else{
